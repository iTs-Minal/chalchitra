...
export async function POST(request: NextRequest) {
  try {
    ...
    const existingReaction = await prisma.movieReviewReaction.findUnique({
      where: {
        userId_reviewId: {
          userId,
          reviewId
    ...
    if (existingReaction) {
      ...
      if (existingReaction.isLike === isLike) {
        await prisma.movieReviewReaction.delete({
        ...
        const updatedReaction = await prisma.movieReviewReaction.update({
          where: { id: existingReaction.id },
          data: { isLike }
        });
      ...
      // Create new reaction
      const newReaction = await prisma.movieReviewReaction.create({
        data: {
          userId,
          reviewId,
          isLike
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (userId) {
      userReaction = await prisma.movieReviewReaction.findUnique({
        where: {
          userId_reviewId: {
            userId,
            reviewId
